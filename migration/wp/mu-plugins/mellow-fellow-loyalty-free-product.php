...
function mellow_fellow_loyalty_free_product($request) {
    ...
    if (!mellow_fellow_free_product_verify_token($params)) {
        error_log('[MF Loyalty Free Product] token verification failed cart_id=' . $params['cart_id'] . ' variant_id=' . $params['variant_id'] . ' point_redemption_id=' . $params['point_redemption_id'] . ' email=' . $params['customer_email']);
